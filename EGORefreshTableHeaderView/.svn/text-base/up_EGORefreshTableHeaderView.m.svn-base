...
- (void)setState:(up_EGOPullRefreshState)aState;
@end
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		[self setState:up_EGOOPullRefreshNormal];
...
- (void)up_refreshLastUpdatedDate {
	...
	if ([_delegate respondsToSelector:@selector(up_egoRefreshTableHeaderDataSourceLastUpdated:)]) {
		
		NSDate *date = [_delegate up_egoRefreshTableHeaderDataSourceLastUpdated:self];
		...
		[[NSUserDefaults standardUserDefaults] setObject:_lastUpdatedLabel.text forKey:@"up_EGORefreshTableView_LastRefresh"];
...
- (void)setState:(up_EGOPullRefreshState)aState{
	...
	switch (aState) {
		case up_EGOOPullRefreshPulling:
			...
			break;
		case up_EGOOPullRefreshNormal:
			
			if (_state == up_EGOOPullRefreshPulling) {
				[CATransaction begin];
				[CATransaction setAnimationDuration:FLIP_ANIMATION_DURATION];
			...
			[self up_refreshLastUpdatedDate];
			
			break;
		case up_EGOOPullRefreshLoading:
...
- (void)up_egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {	
	...
	if (_state == up_EGOOPullRefreshLoading) {
		...
		BOOL _loading = NO;
		if ([_delegate respondsToSelector:@selector(up_egoRefreshTableHeaderDataSourceIsLoading:)]) {
			_loading = [_delegate up_egoRefreshTableHeaderDataSourceIsLoading:self];
		}
		
		if (_state == up_EGOOPullRefreshPulling && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
			[self setState:up_EGOOPullRefreshNormal];
		} else if (_state == up_EGOOPullRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight  && !_loading) {
			[self setState:up_EGOOPullRefreshPulling];
		}
...
- (void)up_egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
		...
		if ([_delegate respondsToSelector:@selector(up_egoRefreshTableHeaderDidTriggerRefresh:)]) {
			[_delegate up_egoRefreshTableHeaderDidTriggerRefresh:self];
		}
		
		[self setState:up_EGOOPullRefreshLoading];
